...
**Описание:** Реализована функция построения вложенного n-мерного массива размером s. Рекурсивная версия (`rec`) на каждом уровне вызывает сама себя, уменьшая глубину. Нерекурсивная версия (`mas`) имитирует это поведение вручную через стек (`list`). Оба варианта выводят одинаковую структуру с элементами `'level n'` на последнем уровне.

<img width="490" height="513" alt="image" src="https://github.com/user-attachments/assets/e89b8d8e-4c14-4890-852f-1a850720317a" />

**Вывод:** создал с помощью функции `def` рекурсивный и нерекурсивный способы
...
**Описание:** Реализованы вспомогательная функция `br(k, x)` и основная `yr(k, x)` для рекурсивного вычисления произведения. Нерекурсивная версия `y(k, x)` выполняет те же вычисления через цикл `for`. Оба метода возвращают одинаковый результат.
