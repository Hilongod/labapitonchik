...
Реализована функция-генератор `gen(*sequences)`, которая объединяет несколько последовательностей по стратегии `interleave` — чередует элементы из каждой по одному, пока все итераторы не исчерпаются. Функция `reduc(sequence)` принимает результат генератора и сворачивает его через `reduce`: если элементы числа — возвращает сумму, если строки — склеивает через пробел. Функция `mr(*sequences)` объединяет оба шага в один вызов.
